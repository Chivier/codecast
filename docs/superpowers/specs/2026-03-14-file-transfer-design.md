...
Discord User
  └─ Sends message + attachment(report.pdf)
      └─ bot_discord.py: on_message()
          ...
          │   → Downloads to ~/.remote-claude/file-pool/abc123_report.pdf
          ...
          └─ _forward_message_with_heartbeat(text, file_refs=[entry])
              ...
              │   │   → SCP to /tmp/remote-claude/files/abc123_report.pdf
              │   │   → Returns {"abc123": "/tmp/remote-claude/files/abc123_report.pdf"}
              │   └─ Replaces: "Analyze this /tmp/remote-claude/files/abc123_report.pdf"
...
│   send_message()       │     │   /tmp/remote-claude/│
...
file_pool:
  ...
  pool_dir: ~/.remote-claude/file-pool  # Local file cache directory
  remote_dir: /tmp/remote-claude/files  # Remote file staging directory
...
3. Verify: file appears in remote `/tmp/remote-claude/files/`
